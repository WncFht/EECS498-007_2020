...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    loss += ((self.params['W1']**2).sum() + (self.params['W2']**2).sum()) * self.reg
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i, hidden_dim in enumerate(hidden_dims):
        j = i + 1
        self.params[f'W{j}'] = torch.zeros(last_dim, hidden_dim, dtype=dtype, device=device)
        self.params[f'W{j}'] += weight_scale*torch.randn(last_dim, hidden_dim, dtype=dtype,device= device)
        self.params[f'b{j}'] = torch.zeros(hidden_dim, dtype=dtype,device= device)
        last_dim = hidden_dim
    j += 1
    self.params[f'W{j}'] = torch.zeros(last_dim, num_classes, dtype=dtype,device= device)
    self.params[f'W{j}'] += weight_scale*torch.randn(last_dim, num_classes, dtype=dtype,device= device)
    self.params[f'b{j}'] = torch.zeros(num_classes, dtype=dtype,device= device)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
        out, cache_dict[i] = Linear_ReLU.forward(out, self.params[f'W{i}'], self.params[f'b{i}'])
        if self.use_dropout:
            out, cache_dict[f'dropout{i}'] = Dropout.forward(out, self.dropout_param)
    scores, cache_dict[self.num_layers] = Linear.forward(out, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in range(self.num_layers, 0, -1):
        if i == self.num_layers:
            dx, dw, db = Linear.backward(dout, cache_dict[i])
        else:
            if self.use_dropout:
                dx = Dropout.backward(dx, cache_dict[f'dropout{i}'])
            dx, dw, db = Linear_ReLU.backward(dx, cache_dict[i])
